...
def bottleneck_block_v1(cnn, depth, depth_bottleneck, stride, augmented, do_downsample=False):
  ...
  with tf.variable_scope(name):
    ...
    if not augmented:
      ...
      cnn.asconv(depth_bottleneck, 3, 3, 1, 1, mode='SAME_RESNET',
                 do_downsample=do_downsample, use_batch_norm=True, bias=None)
...
def bottleneck_block(cnn, depth, depth_bottleneck, stride, version, augmented, do_downsample):
  ...
  if version == 'v2':
    ...
    bottleneck_block_v1(cnn, depth, depth_bottleneck, stride, augmented, do_downsample)
...
class ResnetModel(model_lib.CNNModel):
  ...
  def add_inference(self, cnn):
    ...
    for _ in xrange(self.layer_counts[0]):
      bottleneck_block(cnn, 256, 64, 1, self.version, False, False)
    for i in xrange(self.layer_counts[1]):
      stride = 2 if i == 0 else 1
      bottleneck_block(cnn, 512, 128, stride, self.version, True, True)
    for i in xrange(self.layer_counts[2]):
      stride = 2 if i == 0 else 1
      bottleneck_block(cnn, 1024, 256, stride, self.version, True, False)
    for i in xrange(self.layer_counts[3]):
      stride = 2 if i == 0 else 1
      bottleneck_block(cnn, 2048, 512, stride, self.version, True, False)
    if self.version == 'v2':
      cnn.batch_norm()
...
def create_asresnet50_model(params):
  return ResnetModel('asresnet50', (3, 4, 6, 3), params=params)
...
def create_asresnet101_model(params):
  return ResnetModel('asresnet101', (3, 4, 23, 3), params=params)
...
def create_asresnet152_model(params):
  return ResnetModel('asresnet152', (3, 8, 36, 3), params=params)
